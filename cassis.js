...
function auto_link_re() {
  return '/(?:\\^[0-9]{1,2})|(?:\\@[_a-zA-Z0-9]{1,17})(?:\\@[a-zA-Z0-9][-a-zA-Z0-9]+)?(?:\\.[a-zA-Z0-9][-a-zA-Z0-9]+)*|(?:(?:(?:(?:http|https|irc)?:\\/\\/(?:(?:[!$&-.0-9;=?A-Z_a-z]|(?:\\%[a-fA-F0-9]{2}))+(?:\\:(?:[!$&-.0-9;=?A-Z_a-z]|(?:\\%[a-fA-F0-9]{2}))+)?\\@)?)?(?:(?:(?:[a-zA-Z0-9][-a-zA-Z0-9]*\\.)+(?:(?:aero|app|arpa|asia|a[cdefgilmnoqrstuwxz])|(?:biz|blog|b[abdefghijmnorstvwyz])|(?:cafe|cat|cloud|coffee|com|coop|c[acdfghiklmnoruvxyz])|(?:design|dev|d[ejkmoz])|(?:edu|e[cegrstu])|(?:fyi|f[ijkmor])|(?:gov|g[abdefghilmnpqrstuwy])|h[kmnrtu]|(?:info|int|i[delmnoqrst])|j[emop]|k[eghimnrwyz]|(?:lol|l[abcikrstuvy])|(?:mil|museum|m[acdeghklmnopqrstuvwxyz])|(?:name|net|n[acefgilopruz])|(?:org|om)|(?:party|pro|pub|p[aefghklmnrstwy])|qa|(?:rocks|r[eouw])|(?:social|space|s[abcdeghijklmnortuvyz])|(?:tech|tel|travel|t[cdfghjklmnoprtvwz])|u[agkmsyz]|v[aceginu]|(?:world|wtf|w[fs])|xyz|y[etu]|(?:zone|z[amw])))|(?:(?:25[0-5]|2[0-4][0-9]|[0-1][0-9]{2}|[1-9][0-9]|[1-9])\\.(?:25[0-5]|2[0-4][0-9]|[0-1][0-9]{2}|[1-9][0-9]|[0-9])\\.(?:25[0-5]|2[0-4][0-9]|[0-1][0-9]{2}|[1-9][0-9]|[0-9])\\.(?:25[0-5]|2[0-4][0-9]|[0-1][0-9]{2}|[1-9][0-9]|[0-9])))(?:\\:\\d{1,5})?)(?:\\/(?:(?:[!#&-;=?-Z_a-z~])|(?:\\%[a-fA-F0-9]{2}))*)?)(?=\\b|\\s|$)/';
  ...
  for ($i=0; $i<$mlen; $i++) {
                   ...
                   strcat($mi, '@'))))
    {
                 ...
                 && ($yvid = offset('watch?v=', $mi)) !== 0))
      {
        ...
        if ($mi[0] === '@') { // link all @-domain @-@ with https
          $wmi = substr($mi, 1);
          $ao = offset('@', $wmi);
          if ($ao > 0) { // if there’s a second @, treat it as an @-@ address
            if (substr($wmi, 0, $ao-1) == substr($wmi, $ao)) { // if @domain@domain
              // then link only the domain after it.
              $wmi = strcat('https://', substr($wmi, $ao));
            } else {
              $wmi = strcat('https://', substr($wmi, $ao), '/@',  
                            substr($wmi, 0, $ao-1));
            }
